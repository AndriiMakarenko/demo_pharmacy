...
from pharmacy.core.auth import verify_token
...
async def get_task(_: Annotated[None, Depends(verify_token)]):
    task = get_next_task()
    if task is None:
...
async def update_task(task_id: str, status_update: dict, _: Annotated[None, Depends(verify_token)]):
